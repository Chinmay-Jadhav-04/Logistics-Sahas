...
const ChatPopupScreen = ({ conversation, onClose }) => {
  ...
  // Mock messages data based on the design
  useEffect(() => {
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg) => (
          <div
            key={msg.id}
            className={`flex gap-2 ${msg.sender === 'gol' ? 'justify-end' : 'justify-start'}`}
          >
            {msg.sender === 'user' && (
            ...
            {msg.sender === 'gol' && (
              <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center text-white font-semibold text-xs flex-shrink-0">
                GOL
              </div>
            )}
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 relative">
            <input
              ...
              className="w-full px-4 py-2 bg-gray-100 rounded-full border-none focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white transition-colors"
            />
